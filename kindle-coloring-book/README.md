...
**KDPの品質チェック対応済み:**
- 全ページのテキスト（フォリオ番号・デザイン番号・タイトル文字など）を、
  KDPの最小マージン（0.25in）を満たす位置に修正済みです。
- 使用フォントは Liberation Sans（Helvetica互換）をPDFに埋め込み済みです
  （reportlabが自動追加する未使用の非埋め込みHelveticaリソースも
  `initialFontName` 指定で除去しています）。
  KDPアップロード画面で出ていた「余白からはみ出す」「フォント未埋め込み」
  の指摘は、このリポジトリの最新版では解消されています。
